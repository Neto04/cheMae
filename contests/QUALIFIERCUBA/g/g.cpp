...
bool is_ancestor(int u, int v) {
    return (vis[u] <= vis[v] and low[u] >= low[v]);
}
...
int main() {
    ...
    while (tC--) {
        ...
        // for (int i = 1; i <= n; i++) {
        //     if (AP[i]) cout<<i<<" es AP"<<endl;
        // }

        // Uso vis y low como tin y tout
        t = 0;
        vis.assign(n+1, 0), low.assign(n+1, 0);
        dfs2(s);
        vector<int> ans;
        ans.push_back(s), ans.push_back(e);
        for (int i = 1; i <= n; i++) {
            if (AP[i] and i != s and i != e and is_ancestor(i,e)) ans.push_back(i);
        }
