...
bool check(double radio) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Arriba y abajo
        if (datillos.u > y and datillos.d < 0) return false;
        // Izq y abajo
        if (datillos.l < 0 and datillos.d < 0) return false;
        // Izq y der
        if (datillos.l < 0 and datillos.r > x) return false;
        // Arriba y der
        if (datillos.u > y and datillos.r > x) return false;
    }
    return true;
}
