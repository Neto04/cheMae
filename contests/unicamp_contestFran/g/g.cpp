...
struct Lazy {
    ...
    static Lazy zero() {return {1};}
    // retorna si esa vara es igual al neutro del lazy
    bool is_zero() { return x == 1; } /// moralmente `*this == Lazy::zero()`
};
...
struct Mono { 
	...
	void apply_lazy(Lazy d, int l, int r) { x = (x*d.x)%MOD; }
};

// para mezclar dos lazies
Lazy operator+ (Lazy a, Lazy b) { return {(a.x * b.x)%MOD}; }
// para mezclar dos Monos
Mono operator+ (Mono a, Mono b) { return {(a.x + b.x)%MOD}; }
...
int main() {
    ...
    forn(i,n) {
        ...
        if (x==1) lastUno++;
            ...
            if (!lastUno) continue;
            st.update(0, lastUno, {2});
        }
        else {
            if (lastUno) ans=(ans+st.query(0, lastUno).x-lastUno)%MOD;
        }
    }
    cout<<ans<<"\n";
