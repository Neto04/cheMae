...
int main() {
    ...
    // me dice si la vara es circulo o cuadrado
    ...
    adj.assign(2*n+1, vector<ii> (2*m+1));
    ...
    forn(i,2*n+1) {
        forn(j, 2*n+1) {
            // el mae es centro de un cuadrado y eso no importas
            if (i%2 and j%2) continue;
            // el mae es un punto medio de un lado vertical
            if (i%2) {
                adj[i].push_back({i-1, 5.0});
                adj[i].push_back({i+1, 5.0});
                if (M[i/2][j/2]) adj[i].
            }
